use crate::{dsl::*, signal_::BufferedSignal, signal_player::SignalPlayer};
...
fn make_key_synth(frequency_hz: f64, gate: BufferedSignal<bool>) -> BufferedSignal<f64> {
    ...
    let osc = sum(vec![
        waveform.oscillator(const_(frequency_hz / 2.0), const_(0.5)),
        waveform
            .oscillator(const_(frequency_hz / 4.0), const_(0.5))
            .map(|x| x * 0.5),
        waveform
            .oscillator(const_(frequency_hz * 1.0), const_(0.5))
            .map(|x| x * 0.25),
        waveform
...
impl Component for GuiComponent {
    ...
    fn update(&mut self, state: &mut Self::State, _ctx: Ctx, event: Event) -> Self::Output {
        ...
        if event.tick().is_some() {
            ...
            state.signal_player.send_signal_(&mut state.signal);
        }
    }
